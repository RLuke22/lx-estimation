...
def histogram_predict(belief, left_encoder_ticks, right_encoder_ticks, grid_spec, robot_spec, cov_mask):
    ...
    d_left = robot_spec["wheel_radius"] * dphi_left
    d_right = robot_spec["wheel_radius"] * dphi_right 
    w = (d_right - d_left) / (2 * robot_spec["wheel_baseline"])
    v = np.sin(w) * (d_left + d_right) / 2 
